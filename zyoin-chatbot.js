...
// ── CONFIG — reads from window.ZYOIN_CONFIG ───────────────────
...
function chat(text, onDone){
  ...
  x.onload = function(){
    try{
      ...
      history.pop();
      onDone('Sorry, something went wrong. Please try again or email us at hello@zyoin.com', true);
    }
  };
  x.ontimeout = x.onerror = function(){
    history.pop();
    onDone('Connection timed out. Please check your internet or email hello@zyoin.com', true);
  };
  x.send(body);
}
...
function addMsg(text, who, isErr){
  ...
  div.innerHTML = '<div class="zm-b'+(isErr?' err':'')+'">'+(text.replace(/\n/g,'<br>'))+'</div><div class="zm-t">'+now()+'</div>';
  msgs.insertBefore(div, typing);
  msgs.scrollTop = msgs.scrollHeight;
}

function showTyping(){ var t=document.getElementById('zara-typing'); t.classList.add('on'); document.getElementById('zara-msgs').scrollTop=9999; }
...
function setQR(opts){
  ...
  (opts||[]).forEach(function(o){
    ...
    b.className='zq'; b.textContent=o;
...
function build(){
  ...
  setTimeout(function(){ if(badge.parentNode){ badge.style.transition='opacity .4s'; badge.style.opacity='0'; setTimeout(function(){ badge.style.display='none'; },400); } },6000);
